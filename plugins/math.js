...
let handler  = async (m, { conn, args, usedPrefix }) => {
  if (args.length < 1) return conn.reply(m.chat, 'Modo: noob | fácil | médio | difícil | extremo\n\nExemplos de uso: ' + usedPrefix + 'math médio', m)
  let mode = args[0].toLowerCase()
  if (!(mode in modes)) return conn.reply(m.chat, 'Modo: noob | fácil | médio | difícil | extremo\n\nExemplos de uso: ' + usedPrefix + 'math médio', m)
  let id = m.chat
